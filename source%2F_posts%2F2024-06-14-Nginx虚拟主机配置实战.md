...
在服务器资源有限的情况下，通过虚拟主机功能，可以在一台服务器上托管多个网站，这是运维中的常见需求。Nginx 支持基于域名、IP 和端口的虚拟主机配置，其中基于域名的虚拟主机是最常用的方式。

本文将详细讲解如何配置 Nginx 虚拟主机，实现一个服务器托管多个网站。
...
通过 Nginx 的虚拟主机功能，可以在一台服务器上托管多个网站：

1. **基于域名**：最常用的方式，通过 `server_name` 区分不同网站
2. **配置管理**：使用 sites-available 和 sites-enabled 目录管理配置
3. **HTTPS 支持**：配合 Certbot 实现自动 SSL 证书管理
4. **高级功能**：支持负载均衡、限流、缓存等功能

合理的虚拟主机配置可以充分利用服务器资源，降低运维成本。
